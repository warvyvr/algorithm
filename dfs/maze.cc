...
/*
  the program uses recursion way to perform bfs (depth first search)
...
// it is not function, since it looks like...
...
bool bfs(const maze_t &maze, const point_t &cPoint, const point_t &ePoint, maze_path &path, bool startPoint = false)
{
    ...
    if (find(around_points.begin(), around_points.end(),ePoint) != around_points.end()) {
        path.push_back(cPoint);
        path.push_back(ePoint);
        return true;
    }
